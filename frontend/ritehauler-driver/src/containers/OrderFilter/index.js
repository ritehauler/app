...
class OrderFilter extends Component {
  ...
  constructor(props) {
    ...
    this.onSearchRightButtonPress = this.onSearchRightButtonPress.bind(this);
  ...
  setOrderFilter(
    ...
    if (orderID !== undefined) {
      states["orderID"] = orderID;
    }

    if (selectedStatusIndex !== undefined) {
      states["selectedStatusIndex"] = selectedStatusIndex;
    }

    if (selectedStatusIndex !== undefined && this.searchBar) {
      this.searchBar.setRightText(this.props.statuses[selectedStatusIndex]);
    }

    if (orderID !== undefined && this.searchBar) {
      this.searchBar.setSearchText(orderID);
    }
  ...
  renderSearchBar() {
    return (
      <SearchBar
        ...
        hasRightView
        rightText="Statuses.."
        onRightViewPress={this.onSearchRightButtonPress}
      />
    );
  }

  onSearchRightButtonPress() {
    this.statusesActionSheet.show();
  }
  ...
  handleDoneButtonPress() {
    ...
    if (this.state.selectedStatusIndex !== undefined) activeFiltersCount++;

    if (this.searchBar.getText().length) activeFiltersCount++;
    ...
    if (this.state.isEndDateSelected && !this.state.isStartDateSelected) {
      ...
      this.props.cbOnDone(
        ...
        this.searchBar.getText().length ? this.searchBar.getText() : undefined,
        this.state.selectedOrderStatus
          ? this.state.selectedOrderStatus
          : undefined,
        this.state.selectedStatusIndex !== undefined
  ...
  renderActionSheetModal() {
    return (
      <ActionSheet
        ref={refs => (this.statusesActionSheet = refs)}
        title={""}
        options={this.props.statuses}
        onPress={this.onStatusSelected}
      />
    );
  }

  onStatusSelected = index => {
    if (index !== this.props.orderStatuses.length) {
      this.setState({
        selectedOrderStatus: this.props.orderStatuses[index].entity_id,
        selectedStatusIndex: index
      });
      this.searchBar.setRightText(this.props.statuses[index]);
    } else {
      this.setState({
        selectedOrderStatus: undefined,
        selectedStatusIndex: undefined
      });
      this.searchBar.setRightText("Statuses..");
    }
  };
...
const mapStateToProps = ({ orderStatuses }) => {
  return {
    orderStatuses: orderStatuses.data,
    statuses: constructOrderStatusesArray(orderStatuses.data)
  };
};
const actions = {};
