...
class CCUser extends Base {
    ...
    protected $hidden = ['password', 'remember_login_token'];
	 ...
	 public function __construct() {
        ...
        $this->hidden = array();
		// entity vars
		$this->_entity_identifier = "cc_user";
		$this->_entity_session_identifier = config('pl_cc_user.SESS_KEY');
		$this->_entity_dir = config('pl_cc_user.DIR_PANEL');
		$this->_entity_pk = "user_id";
		$this->_entity_salt_pattren = config('pl_cc_user.SALT');
		$this->_entity_model = $this;

        // set fields
        $this->__fields = array($this->primaryKey,'name','first_name','last_name','user_name','email','password','dob','gender','image','thumb','status','platform_type','device_udid','device_type','device_type','device_token','is_verified','verification_token', 'verified_at', 'is_mobile_verified','mobile_verification_code','mobile_verified_at','is_guest','additional_note','other_data','remember_login_token', 'remember_login_token_created_at', 'forgot_password_hash', 'forgot_password_hash_created_at','last_login_at','last_seen_at','created_at','updated_at','deleted_at');
    }
    ...
    function setRememberToken($data) {
		...
		// set cookie
		setcookie($this->_entity_session_identifier."remember_login_token", $remember_login_token, time() + (config('pl_cc_user.REMEMBER_COOKIE_TIME')), "/");
		// return token
    ...
    function removeRememberToken($data) {
		if($data) {
			...
			// forget cookie
			setcookie($this->_entity_session_identifier."remember_login_token", FALSE, time() - (config('pl_cc_user.REMEMBER_COOKIE_TIME')), "/");
		}
	}
    ...
    function signup($data) {
		...
		$data->password = $this->saltPassword($data->password);
		 // generate hash
		$data->verification_token = str_random(config('pl_'.$this->_entity_identifier.'.SIGNUP_HASH_LENGTH') / 2);
		$data->is_verified = 0;
		...
		# load email template
		$email_template = $email_template_model->getBy('key', $this->_entity_identifier.'_signup_confirmation');
		...
		$wildcard['replace'] = array(
			...
			//\URL::to($this->_entity_dir."confirm_signup/?email=".$data->email."&hash=".$data->verification_token), // CONFIRMATION_LINK
			\URL::to($this->_entity_identifier."/confirm_signup/?email=".$data->email."&hash=".$data->verification_token), // CONFIRMATION_LINK
    ...
    function generateNew($data) {
        ...
        // temporary password
		$data->password = str_random(config('pl_cc_user.FORGOT_PASS_HASH_LENGTH') / 2);
    ...
    function forgotPassword($data) {
		...
		// get code
		$code = str_random(config('pl_cc_user.FORGOT_PASS_HASH_LENGTH'));
		...
		$wildcard['replace'] = array(
			...
			\URL::to($this->_entity_identifier."/confirm_forgot/?code=".$code."&email=".$data->email), // CONFIRMATION_LINK
		);
		# subject
		$data->subject = str_replace($wildcard['key'], $wildcard['replace'], $email_template->subject);
		# body
    ...
    function welcomeUser($user) {
		...
		$data['verification_token'] = NULL; // null
        $data['is_verified'] = 1; // verified ?
		$data['verified_at'] = date('Y-m-d H:i:s'); // verified at
        // reserve temp password
        //$temp_password = $data['temp_password'];
        //$data['temp_password'] = '';

        // update user record
        $this->set($data[$this->__fields[0]], $data);
        // send email to new admin
        # un-encoded password
        ...
        # site data
        //$data['site'] = $conf_model->getBy('key','site');
        //$data['site'] = json_decode($data['site']->value);
        # load email template
        //$body = $this->load->view('email/welcome_email_template.php', $data, true);
        # load email template
		$email_template = $email_template_model->getBy('key', $this->_entity_identifier.'_signup_welcome');
        $wildcard['key'] = explode(',', $email_template->wildcards);
        $wildcard['replace'] = array(
            $conf->site_name, // APP_NAME
            url("/") . "/", // APP_LINK
			//$data['name'],
			($data['first_name'].' '.$data['last_name']), // USER_NAME
        );
        ...
        if (!preg_match("@localhost@", url("/"))) {
            $this->sendMail(
				array($data["email"], $data['first_name'].' '.$data['last_name']),
				$body,
				$data
			);
        }
    ...
    public function getData($id = 0) {
		...
		if($data) {
			// unset unrequired
			unset($data->deleted_at,$data->user_name,$data->password,$data->verification_token,$data->mobile_verification_code, $data->remember_login_token, $data->remember_login_token_created_at, $data->forgot_password_hash, $data->forgot_password_hash_created_at);
