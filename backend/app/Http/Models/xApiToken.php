...
class xApiToken extends Base {
    ...
    public function getDataByToken($token, $do_update_data = 0, $entity_type = NULL, $entity_id = NULL) {
		...
		$raw_query = $this->select($this->primaryKey)
			...
			//->whereRaw("expire_at > '".date("Y-m-d H:i:s")."'")
			->where("token","=",$token)
		...
		if($data !== FALSE) {
			...
			if($do_update_data > 0) {
				...
				if($data->entity_type !== NULL && $data->entity_id !== NULL) {
					// entity vars
					$entity_model = $this->_entity_models[$data->entity_type];
					$entity_pk = $this->_entity_pks[$data->entity_type];
					// init model
					$entity_model = $this->_model_path.$entity_model;
					$entity_model = new $entity_model;
					// entity data
					$entity = $entity_model->get($data->entity_id);
					// if found
					if($entity !== FALSE) {
						// assign last seen and update
						$entity->last_seen_at = date("Y-m-d H:i:s");
						$entity_model->set($entity->{$entity_pk},(array)$entity);
					}
				}
    ...
    function generate($entity_type = NULL, $entity_id = NULL) {
		...
		if($entity_type !== NULL && $entity_id !== NULL) {
			...
			$entity_model = $this->_entity_models[$entity_type];
			// load entity model
			$entity_model = $this->_model_path.$entity_model;
			$entity_model = new $entity_model;
			// get entity record
			$entity_record = $entity_model->get($entity_id);
			// if valid entity
