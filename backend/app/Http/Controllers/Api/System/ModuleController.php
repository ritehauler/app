...
class ModuleController extends Controller
{
    ...
    private $_object_identifier = "modules";
    private $_entity_identifier = "system_module"; // usually routes path
    private $_entity_pk = "module_id";
    private $_entity_ucfirst = "Module";
    private $_entity_model = "SYSModule";
    ...
    public function post(Request $request)
    {
        ...
        // extra models
        $exModel = $this->_model_path . "SYSStatus";
        $exModel = new $exModel;

        $request->parent_id = intval($request->input("parent_id",0)) > 0 ? intval($request->input("parent_id",0)) : NULL;
        // validations
        $rules = array(
            'parent_id' => 'integer:exists:' . $this->_entity_model->table . "," . $this->_entity_model->primaryKey . ",deleted_at,NULL",
            'title' => 'bail|required|string',
            'identifier' => "bail|required|string|unique:".$this->_entity_model->table.",".$this->_entity_model->primaryKey,
            'css_class' => 'string',
            'slug' => 'string',
            'is_active' => "bail|required|integer",
            'json_data' => 'string',
            'sequence' => 'integer',
        );
        ...
        if ($validator->fails()) {
            ...
            $entity["parent_id"] = intval($entity["parent_id"]) > 0 ? intval($entity["parent_id"]) : NULL;
            $entity["sequence"] = intval($request->input("sequence", 0)) > 0 ? intval($request->input("sequence", 0)) : 0;
    ...
    public function update(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            // set data types for optional fields
            $entity["parent_id"] = intval($entity["parent_id"]);
            $entity["sequence"] = intval($request->input("sequence", 0)) > 0 ? intval($request->input("sequence", 0)) : 0;
    ...
    public function listing(Request $request)
    {
        ...
        $exModel = new $exModel;
        // allowed order
        $allowed_ordering = $allowed_searching = $this->_entity_model->primaryKey . ",parent_id,title,identifier,value,sequene,created_at";
        ...
        $rules = array(
            ...
            'parent_id' => 'integer:exists:' . $this->_entity_model->table . "," . $this->_entity_model->primaryKey . ",deleted_at,NULL",
            'title' => 'string',
            'identifier' => "string|exists:".$this->_entity_model->table.",".$this->_entity_model->primaryKey.",deleted_at,NULL,parent_id,".$request->parent_id,
            "css_class" => "string",
            'status_id' => 'integer|exists:'.$exModel->table.','.$exModel->primaryKey.',deleted_at,NULL',
            'json_data' => 'string',
            "sequence" => "integer",
    ...
    {
        // fix indexes
        $fix_indexes = array($this->_entity_pk, "parent_id", "status_id", "sequence");
        // search
