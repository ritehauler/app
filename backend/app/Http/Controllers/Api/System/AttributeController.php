...
class AttributeController extends Controller
{
    ...
    public function get(Request $request)
    {
        ...
        if ($request->{$this->_attribute_pk} == 0) {
            $this->_apiData['message'] = trans('api_errors.pls_enter_attribute_id', array("attribute" => $this->_object_identifier));
    ...
    public function delete(Request $request)
    {
        ...
        if ($validator->fails()) {
			...
			if(!$check_exists){
				...
				$this->_apiData['message'] = trans('api_errors.attribute_delete_success');
				// response data
            	// assign to output
            	$this->_apiData['data'] = $data;
			}else{
				$this->_apiData['message'] = trans('api_errors.attribute_in_use');
			}
