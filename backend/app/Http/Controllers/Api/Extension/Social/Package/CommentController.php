...
class CommentController extends Controller
{
    ...
    private $_activityTypeData; // activity type data
    private $_extIdentifier = "ext_social_package";
    private $_activityIdentifier = "comment";
    private $_activityTypeModel = "ExtActivityType"; // extension required model
    private $_pModel = "ExtPackageComment"; // extension model
    private $_pHook = "ExtPackageComment"; // extension hook
    ...
    public function post(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), array(
            ...
            'parent_id' => 'int|exists:' . $this->_pModel->table . "," . $this->_pModel->primaryKey . ",deleted_at,NULL",
            // comment
            'content' => 'required_without:json_data',
            // json data
            "json_data" => "required_without:content|json",
        ...
        if ($validator->fails()) {
            ...
            $this->_apiData['message'] = trans($this->_extIdentifier . '.already_liked', array("entity" => $entity_type->identifier));
        } elseif ($request->switch == 0 && $id == 0) {
            $this->_apiData['message'] = trans($this->_extIdentifier . '.have_not_liked', array("entity" => $entity_type->identifier));
        }*/ else {
            // success response
            ...
            // api msg
            if (intval($request->input('parent_id', 0)) > 0) {
                $this->_apiData['message'] = trans($this->_extIdentifier . '.reply_insert_success');
            } else {
                $this->_apiData['message'] = trans($this->_extIdentifier . '.comment_insert_success');
            }
            ...
            // - identifier
            if (intval($entity->parent_id) > 0) {
                $h_identifier = "reply_add";
            } else {
                $h_identifier = "comment_add";
            }
            // - log
            $other_data = array(
    ...
    public function update(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            // api msg
            if (intval($entity['parent_id']) > 0) {
                $this->_apiData['message'] = trans($this->_extIdentifier . '.reply_update_success');
            } else {
                $this->_apiData['message'] = trans($this->_extIdentifier . '.comment_update_success');
            }
            ...
            // - identifier
            if (intval($entity->parent_id) > 0) {
                $h_identifier = "reply_" . __FUNCTION__;
            } else {
                $h_identifier = "comment_" . __FUNCTION__;
            }
    ...
    public function listing(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), array(
            ...
            $this->_pModel->primaryKey => 'filled|int|exists:' . $this->_pModel->table . ',' . $this->_pModel->primaryKey . ',actor_' . $this->_entityModel->primaryKey . ',' . $request->input('actor_' . $this->_entityModel->primaryKey, 0) . ',' . $this->_eTypeExtMapModel->primaryKey . ',' . $this->_extMapData->{$this->_eTypeExtMapModel->primaryKey} . ',deleted_at,NULL',
        ));
        ...
        if ($validator->fails()) {
            ...
            $query = $this->_pModel->select($primary_alias . $this->_pModel->primaryKey)
                ...
                ->where($primary_alias . 'target_' . $this->_entityModel->primaryKey, $request->{'target_' . $this->_entityModel->primaryKey})
