...
class RateController extends Controller
{
    ...
    private $_activityIdentifier = "rate";
    private $_activityTypeModel = "ExtActivityType"; // extension required model
    private $_pModel = "ExtPackageRate"; // extension model
    private $_pHook = "ExtPackageRate"; // extension hook
    ...
    public function post(Request $request)
    {
      ...
      //  $request->merge(array_map('trim', $request->all()));
        ...
        if ($validator->fails()) {
            ...
            $this->_apiData['message'] = trans($this->_extIdentifier . '.already_rated', array("entity" => $entity_type->identifier));
        } else {
            // success response
            ...
            // insert
            $id = $this->_pModel->saveData($this->_extMapData, $save, $timestamp,$request->all());
    ...
    public function listing(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $query = $this->_pModel->select($primary_alias . $this->_pModel->primaryKey)
                ...
                ->join($this->_entityModel->table . ' AS b', 'b.' . $this->_entityModel->primaryKey, '=', $primary_alias . 'target_' . $this->_entityModel->primaryKey)
            ...
            // extra node to show user review on top
            $data["review"] = $this->_pModel->check(
