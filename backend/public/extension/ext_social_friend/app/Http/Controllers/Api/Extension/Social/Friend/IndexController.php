<?php
namespace App\Http\Controllers\Api\Extension\Social\Friend;
...
class IndexController extends Controller
{
    ...
    private $_extIdentifier = "{plugin_identifier}";
    private $_pModel = "ExtSocialFriend"; // extension model
    private $_pHook = "ExtSocialFriend"; // extension hook
    ...
    public function post(Request $request)
    {
        ...
        $query = $this->_pModel
            ->whereRaw("
            ((target_" . $this->_entityModel->primaryKey . " = '" . $request->{'target_' . $this->_entityModel->primaryKey} . "' && actor_" . $this->_entityModel->primaryKey . " = '" . $request->{"actor_" . $this->_entityModel->primaryKey} . "')
            OR
            (target_" . $this->_entityModel->primaryKey . " = '" . $request->{'actor_' . $this->_entityModel->primaryKey} . "' && actor_" . $this->_entityModel->primaryKey . " = '" . $request->{"target_" . $this->_entityModel->primaryKey} . "'))")
            ->whereIn('request_status', array('pending', 'accepted'))
            ->whereNull('deleted_at');
        $row_type_exists = $query->get();
        $id = isset($row_type_exists[0]) ? $row_type_exists[0]->{$this->_pModel->primaryKey} : 0;
        // fetch record
        $record = json_decode(json_encode($this->_pModel->get($id)));

        // validate
        if ($validator->fails()) {
            $this->_apiData['message'] = $validator->errors()->first();
        } elseif ($record && $record->request_status == 'accepted') {
            $this->_apiData['message'] = trans($this->_extIdentifier . '.already_friends', array("entity" => $entity_type->identifier));
        } elseif ($record && ($record->{'actor_' . $this->_entityModel->primaryKey} == $request->{'actor_' . $this->_entityModel->primaryKey})) {
            $this->_apiData['message'] = trans($this->_extIdentifier . '.request_pending', array("entity" => $entity_type->identifier));
        } elseif ($record && ($record->{'target_' . $this->_entityModel->primaryKey} == $request->{'actor_' . $this->_entityModel->primaryKey})) {
            $this->_apiData['message'] = trans($this->_extIdentifier . '.plz_accept_or_reject', array("entity" => $entity_type->identifier));
        } else {
            // success response
            ...
            // api msg
            $this->_apiData['message'] = trans($this->_extIdentifier . '.request_sent', array("entity" => $entity_type->identifier));
            ...
            // - identifier
            $h_identifier = $this->_pModel->objectIdentifier . "_add";
            // - log
            $other_data = array(
    ...
    public function cancel(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), array(
            // actory entity vs actor entity type
            'actor_' . $this->_entityModel->primaryKey => 'required|int|exists:' . $this->_entityModel->table . ',' . $this->_entityModel->primaryKey . ',' . $this->_entityTypeModel->primaryKey . ',' . $request->input('actor_' . $this->_entityTypeModel->primaryKey) . ',deleted_at,NULL',
            // check record matches with actor
            $this->_pModel->primaryKey => 'required|int|exists:' . $this->_pModel->table . ',' . $this->_pModel->primaryKey . ',actor_' . $this->_entityModel->primaryKey . ',' . $request->input('actor_' . $this->_entityModel->primaryKey) . ',request_status,pending,deleted_at,NULL',
        ));
        ...
        if ($validator->fails()) {
            ...
            $record = $this->_pModel->get($id);
            $save = (array)$record;

            // prepare data
            $save['request_status'] = 'cancelled';
            $save['updated_at'] = $timestamp;
            // cancel
            $this->_pModel->cancelRequest($this->_extMapData, $request->{$this->_pModel->primaryKey}, $save, $timestamp);

            // api msg
            $this->_apiData['message'] = trans($this->_extIdentifier . '.request_cancelled', array("entity" => $entity_type->identifier));
    ...
    public function delete(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), array(
            ...
            'actor_' . $this->_entityModel->primaryKey => 'required|int|exists:' . $this->_entityModel->table . ',' . $this->_entityModel->primaryKey . ',' . $this->_entityTypeModel->primaryKey . ',' . $request->input('actor_' . $this->_entityTypeModel->primaryKey) . ',deleted_at,NULL',
            // check record matches with actor
            $this->_pModel->primaryKey => 'required|int|exists:' . $this->_pModel->table . ',' . $this->_pModel->primaryKey . ',request_status,accepted,deleted_at,NULL',
        ));

        // get record
        $entity_type = $this->_entityTypeModel->get($this->_extMapData->{'target_' . $this->_entityTypeModel->primaryKey});

        // check existence
        $query = $this->_pModel
            ->where(function ($query) use ($request) {
                $query->where('actor_' . $this->_entityModel->primaryKey, '=', $request->{'actor_' . $this->_entityModel->primaryKey})
                    ->orWhere('target_' . $this->_entityModel->primaryKey, '=', $request->{'actor_' . $this->_entityModel->primaryKey});
        ...
        if ($validator->fails()) {
            ...
            $this->_apiData['message'] = trans($this->_extIdentifier . '.not_friends', array("entity" => $entity_type->identifier));
        } else {
            // success response
            $this->_apiData['response'] = "success";
            ...
            // api msg
            $this->_apiData['message'] = trans($this->_extIdentifier . '.friend_deleted', array("entity" => $entity_type->identifier));
    ...
    public function accept(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), array(
            ...
            'actor_' . $this->_entityModel->primaryKey => 'required|int|exists:' . $this->_entityModel->table . ',' . $this->_entityModel->primaryKey . ',' . $this->_entityTypeModel->primaryKey . ',' . $request->input('actor_' . $this->_entityTypeModel->primaryKey) . ',deleted_at,NULL',
            // check record matches with actor
            $this->_pModel->primaryKey => 'required|int|exists:' . $this->_pModel->table . ',' . $this->_pModel->primaryKey . ',target_' . $this->_entityModel->primaryKey . ',' . $request->input('actor_' . $this->_entityModel->primaryKey) . ',request_status,pending,deleted_at,NULL',
        ));
        ...
        if ($validator->fails()) {
            ...
            // prepare data
            $save['request_status'] = 'accepted';
            $save['updated_at'] = $timestamp;
            // cancel
            $this->_pModel->acceptRequest($this->_extMapData, $request->{$this->_pModel->primaryKey}, $save, $timestamp);

            // api msg
            $this->_apiData['message'] = trans($this->_extIdentifier . '.request_accepted', array("entity" => $entity_type->identifier));
    ...
    public function reject(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            // prepare data
            $save['request_status'] = 'rejected';
            $save['updated_at'] = $timestamp;
            // cancel
            $this->_pModel->rejectRequest($this->_extMapData, $request->{$this->_pModel->primaryKey}, $save, $timestamp);

            // api msg
            $this->_apiData['message'] = trans($this->_extIdentifier . '.request_rejected', array("entity" => $entity_type->identifier));
    ...
    public function get(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), array(
            ...
            'actor_' . $this->_entityModel->primaryKey => 'required|int|exists:' . $this->_entityModel->table . ',' . $this->_entityModel->primaryKey . ',' . $this->_entityTypeModel->primaryKey . ',' . $request->input('actor_' . $this->_entityTypeModel->primaryKey) . ',deleted_at,NULL',
            // check record matches with actor
            $this->_pModel->primaryKey => 'required|int|exists:' . $this->_pModel->table . ',' . $this->_pModel->primaryKey . ',deleted_at,NULL',
        ));
    ...
    public function listing(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), array(
            ...
            'request_status' => 'in:accepted,received,sent'
        ));

        // defaults
        $request_status = $request->input('request_status', '') == '' ? 'accepted' : $request->request_status;
        ...
        if ($validator->fails()) {
            ...
            $query = $this->_pModel->select($primary_alias . $this->_pModel->primaryKey)
                ...
                ->join($this->_entityModel->table . ' AS b', 'b.' . $this->_entityModel->primaryKey, '=', $primary_alias . 'data_' . $this->_entityModel->primaryKey)
            ...
            if ($request_status == 'received') {
                // target
                $query->where($primary_alias . 'target_' . $this->_entityModel->primaryKey, $request->{'actor_' . $this->_entityModel->primaryKey});
                // status
                $query->where($primary_alias . 'request_status', 'pending');
            } else if ($request_status == 'sent') {
                // actor
                ...
                $query->where(function ($query) use ($request, $primary_alias) {
                    $query->where($primary_alias . 'target_' . $this->_entityModel->primaryKey
                        , $request->{'actor_' . $this->_entityModel->primaryKey})
                        ->orWhere($primary_alias . 'actor_' . $this->_entityModel->primaryKey
                            , $request->{'actor_' . $this->_entityModel->primaryKey});
                });
                // status
                $query->where($primary_alias . 'request_status', $request_status);
            }
            // get total
