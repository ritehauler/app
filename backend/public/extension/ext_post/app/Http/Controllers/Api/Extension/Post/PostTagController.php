...
class PostTagController extends Controller
{
    ...
    private $_pModel = "PostTag"; // extension model
    private $_pHook = "PostTag"; // extension hook
    ...
    public function post(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), array(
            ...
            // identifier
            'identifier' => 'required|min:2|unique:' . $this->_pModel->table . ',identifier,NULL,' . $this->_pModel->primaryKey . ',deleted_at,NULL',
    ...
    public function update(Request $request)
    {
        ...
        $validator = Validator::make($request->all(), array(
            // actory entity vs actor entity type
            'actor_' . $this->_entityModel->primaryKey => 'required|int|exists:' . $this->_entityModel->table . ',' . $this->_entityModel->primaryKey . ',' . $this->_entityTypeModel->primaryKey . ',' . $request->input("actor_" . $this->_entityTypeModel->primaryKey),
            $this->_pModel->primaryKey => 'required|int|exists:' . $this->_pModel->table . ',' . $this->_pModel->primaryKey . ",actor_" . $this->_entityModel->primaryKey . ',' . $request->input('actor_' . $this->_entityModel->primaryKey, 0) . ",deleted_at,NULL",
            // title
            'title' => 'required|min:2',
            // identifier
            'identifier' => 'required|min:2|unique:' . $this->_pModel->table . ',identifier,' . $request->{$this->_pModel->primaryKey} . ',' . $this->_pModel->primaryKey . ',deleted_at,NULL',
        ...
        if ($validator->fails()) {
            ...
            // update
            $this->_pModel->updateData($this->_extMapData, $id, (array)$entity, $timestamp);

            // api msg
            $this->_apiData['message'] = trans($this->_extIdentifier . '.entity_update_success', array("entity" => $this->_pModel->objectIdentifier));
    ...
    public function delete(Request $request)
    {
        ...
        $override_access = intval($request->input('override_access', 0)) > 0 ? 1 : 0;
        ...
        if ($override_access > 0) {
            $rules[$this->_pModel->primaryKey] = 'required|int|exists:' . $this->_pModel->table . ',' . $this->_pModel->primaryKey . ',deleted_at,NULL';
        }
        ...
        if ($validator->fails()) {
            ...
            // api msg
            $this->_apiData['message'] = trans($this->_extIdentifier . '.entity_delete_success', array("entity" => $this->_pModel->objectIdentifier));
     ...
     * @return Response
     */
    public function postOrGet(Request $request)
    {
        // trim/escape all
        $request->merge(array_map('strip_tags', $request->all()));
        $request->merge(array_map('trim', $request->all()));
        ...
        // check exists
        $check_exists = $this->_pModel
            ->where('identifier',$request->identifier)
            ->whereNull('deleted_at')
            ->first();
        $check_exists = json_decode(json_encode($check_exists));
        // if exists
        if(isset($check_exists->{$this->_pModel->primaryKey})) {
            // merge key
            $request->merge(array($this->_pModel->primaryKey => $check_exists->{$this->_pModel->primaryKey}));
    ...
    public function listing(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            $allowed_ordering = $this->_pModel->primaryKey . ",title,identifier,created_at";
            $primary_alias = 'a.';
            ...
            if (trim($request->title) != '') {
                $query->where($primary_alias . 'title', 'like', '%' . trim($request->title) . '%');
            }
            // got identifier ?
            if (trim($request->identifier) != '') {
                $query->where($primary_alias . 'identifier', 'like', '%' . trim($request->identifier) . '%');
            }
            // got introduced_by_entity_id ?
            if ($request->input('introduced_by_entity_id', null) != null) {
                $query->where($primary_alias . 'introduced_by_entity_id', intval($request->introduced_by_entity_id));
            }
            // got count_usage from ?
            if ($request->input('count_usage_from', null) != null) {
                $query->where($primary_alias . 'count_usage', '>=', intval($request->count_usage_from));
            }
            // got count_usage to ?
            if ($request->input('count_usage_to', null) != null) {
                $query->where($primary_alias . 'count_usage', '<=', intval($request->count_usage_to));
            }
            // get total
