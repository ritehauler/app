...
class PostController extends Controller
{
    ...
    private $_pModelPath = "Extension\\Post\\";
    ...
    //private $_activityIdentifier = "form_field";
    //private $_activityTypeModel = "ExtActivityType"; // extension required model
    private $_pModel = "FormFieldMap"; // extension model
    private $_pHook = "FormFieldMap"; // extension hook
    ...
    public function post(Request $request)
    {
        ...
        // init models
        $eTypeFormModel = $this->_pModelPath . 'EntityTypeFormMap';
        $eTypeFormModel = new $eTypeFormModel;
        $fieldTypeModel = $this->_pModelPath . 'FieldType';
        ...
        $validator = Validator::make($request->all(), array(
            ...
            // entity_type_form_map_id
            $eTypeFormModel->primaryKey => 'required|int|exists:' . $eTypeFormModel->table . ',' . $eTypeFormModel->primaryKey . ',deleted_at,NULL',
            // field_type_id
            $fieldTypeModel->primaryKey => 'required|int|exists:' . $fieldTypeModel->table . ',' . $fieldTypeModel->primaryKey . ',deleted_at,NULL',
            // field_name
            'field_name' => 'required|string|unique:' . $this->_pModel->table . ',field_name,0,' . $this->_pModel->primaryKey . ',' . $fieldTypeModel->primaryKey . ',' . $request->input($fieldTypeModel->primaryKey, 0) . ',deleted_at,NULL',
            // title
            'title' => 'required',
            // js_validation_type
            'js_validation_type' => 'in:default,custom,regex'
        ));

        // js_validation_type
        $js_validation_type = trim($request->input('js_validation_type', 'default'));
        $js_validation_type = $js_validation_type == '' ? 'default' : $js_validation_type;
        // js_validation_event
        $js_validation_event = trim($request->input('js_validation_event', 'none'));
        $js_validation_event = $js_validation_event == '' ? 'none' : $js_validation_event;
        // php_validation_type
        $php_validation_type = trim($request->input('php_validation_type', 'default'));
        $php_validation_type = $php_validation_type == '' ? 'default' : $php_validation_type;
        // php_validation_rule
        $php_validation_rule = trim($request->input('php_validation_rule', ''));
        $php_validation_rule = $php_validation_rule == '' ? '' : $php_validation_rule;
        // js_validation_rule
        $js_validation_rule = trim($request->input('js_validation_rule', ''));
        $js_validation_rule = $js_validation_rule == '' ? '' : $js_validation_rule;

        // validations (js)
        $validator2 = Validator::make($request->all(), array(
            // js_validation_rule
            'js_validation_rule' => 'required|min:1',
            // js_validation_event
            'js_validation_event' => 'required|in:onblur,onchange,onclick,onfocus,onselect'
        ));
        // validations (php)
        $validator3 = Validator::make($request->all(), array(
            // js_validation_rule
            'php_validation_rule' => 'required|min:1',
        ));

        // validate
        if ($validator->fails()) {
            $this->_apiData['message'] = $validator->errors()->first();
        } elseif ($js_validation_type != 'default' && $validator2->fails()) {
            $this->_apiData['message'] = $validator2->errors()->first();
        } elseif ($php_validation_type != 'default' && $validator3->fails()) {
            $this->_apiData['message'] = $validator3->errors()->first();
        } else {
            // success response
            ...
            // prepare data
            $save = array(
                $eTypeFormModel->primaryKey => $request->{$eTypeFormModel->primaryKey},
                $fieldTypeModel->primaryKey => $request->{$fieldTypeModel->primaryKey},
                'field_name' => $request->input('field_name'),
                'title' => $request->input('title', ''),
                'description' => $request->input('description', ''),
                'hint' => $request->input('hint', ''),
                'html5_input_type' => $request->input('html5_input_type', null),
                'js_validation_type' => $js_validation_type,
                'js_validation_rule' => $js_validation_rule,
                'js_validation_event' => $js_validation_event,
                'php_validation_type' => $php_validation_type,
                'php_validation_rule' => $php_validation_rule,
                "created_at" => $timestamp
            );
            // insert
            $id = $this->_pModel->put($save);

            // api msg
            $this->_apiData['message'] = trans($this->_extIdentifier . '.entity_insert_success', array("entity" => $this->_pModel->objectIdentifier));
    ...
    public function xupdate(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            // api msg
            $this->_apiData['message'] = trans($this->_extIdentifier . '.entity_update_success', array("entity" => $entity_type->identifier));
    ...
    public function xdelete(Request $request)
    {
        ...
        if ($validator->fails()) {
            ...
            // api msg
            $this->_apiData['message'] = trans($this->_extIdentifier . '.entity_delete_success', array("entity" => $entity_type->identifier));
    ...
    public function listing(Request $request)
    {
        ...
        // init model
        $eTypeFormMapModel = $this->_pModelPath.'EntityTypeFormMap';
        $eTypeFormMapModel = new $eTypeFormMapModel;
        $fieldTypeModel = $this->_pModelPath.'FieldType';
        $fieldTypeModel = new $fieldTypeModel;

        // validations
        $validator = Validator::make($request->all(), array(
            // entity type form map
            $eTypeFormMapModel->primaryKey => 'required|int|exists:' . $eTypeFormMapModel->table . ',' . $eTypeFormMapModel->primaryKey . ',deleted_at,NULL',
            // field type
            $fieldTypeModel->primaryKey => 'int|exists:' . $fieldTypeModel->table . ',' . $fieldTypeModel->primaryKey . ',deleted_at,NULL',
        ...
        if ($validator->fails()) {
            ...
            $query = $this->_pModel->select($primary_alias . $this->_pModel->primaryKey)
                ...
                ->where($primary_alias . $eTypeFormMapModel->primaryKey, '=', $request->{$eTypeFormMapModel->primaryKey})
                ->whereNull($primary_alias . 'deleted_at');
            // field type
            if (trim($request->input($fieldTypeModel->primaryKey)) != '') {
                $query->where($primary_alias . $fieldTypeModel->primaryKey, intval($request->{$fieldTypeModel->primaryKey}));
            }
            // field name
            if (trim($request->input('field_name')) != '') {
                $query->where($primary_alias . 'field_name', 'like', '%' . trim($request->input('field_name')) . '%');
            }
            // html5_input_type
            if (trim($request->input('html5_input_type')) != '') {
                $query->where($primary_alias . 'html5_input_type', 'like', '%' . trim($request->input('html5_input_type')) . '%');
            }
            // js_validation_type
            if (trim($request->input('js_validation_type')) != '') {
                $query->where($primary_alias . 'js_validation_type', 'like', '%' . trim($request->input('js_validation_type')) . '%');
            }
            // js_validation_event
            if (trim($request->input('js_validation_event')) != '') {
                $query->where($primary_alias . 'js_validation_event', 'like', '%' . trim($request->input('js_validation_event')) . '%');
            }
            // php_validation_type
            if (trim($request->input('php_validation_type')) != '') {
                $query->where($primary_alias . 'php_validation_type', 'like', '%' . trim($request->input('php_validation_type')) . '%');
            }
            // get total
